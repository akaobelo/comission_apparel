                    ...
                    <div class="p-5 border-b border-slate-200 bg-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-4 cursor-pointer" @click="expanded = !expanded">
                        ...
                        <div class="flex gap-4 items-center">
                            <form method="GET" action="{{ route('admin.dashboard') }}" class="flex gap-2" @click.stop>
                                @foreach(request()->except(['collection_search', 'collection_page']) as $k => $v)
                                    @if(is_array($v))
                                        @foreach($v as $arrVal)
                                ...
                                <div class="relative">
                                    <svg class="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                                    <input type="text" name="collection_search" value="{{ request('collection_search') }}" placeholder="Search collections..." class="pl-9 pr-4 py-2 bg-white border border-slate-300 rounded-lg text-sm text-slate-900 focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none shadow-sm w-full md:w-64">
                                </div>
                                @if(request()->filled('collection_search'))
                                    <a href="{{ route('admin.dashboard', request()->except(['collection_search', 'collection_page'])) }}" class="px-3 py-2 bg-white border border-slate-300 text-slate-700 text-xs font-bold uppercase tracking-wider rounded-lg hover:bg-slate-50 transition-colors flex items-center justify-center">Clear</a>
                                @endif
