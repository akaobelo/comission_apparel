...
@section('meta')
    @php
        ...
        // Prioritize the collection's cover photo (image_path) since it is landscape and renders a large preview card
        if ($collectionModel && !empty($collectionModel->image_path)) {
            $ogImage = $collectionModel->image_path;
        }

        // Fallback to specific design if requested in query
        ...
        // Fallback to the first design in the collection if no cover or specific design is found
        if (!$ogImage) {
...
<section class="py-4 md:py-8 bg-slate-50 min-h-[50vh]" x-data="{ previewOpen: false, previewImgs: [], previewIdx: 0, previewAlt: '', touchStartX: 0, touchEndX: 0 }" @keydown.escape.window="previewOpen = false; document.body.style.overflow = 'auto';" @keydown.right.window="if(previewOpen && previewImgs.length > 1) previewIdx = (previewIdx + 1) % previewImgs.length" @keydown.left.window="if(previewOpen && previewImgs.length > 1) previewIdx = (previewIdx - 1 + previewImgs.length) % previewImgs.length">
    <div class="max-w-[1500px] mx-auto px-6">
        <div class="mb-8 flex flex-col gap-4">
            <form method="GET" action="{{ route('catalog.show', ['collection' => \Illuminate\Support\Str::slug($collection)]) }}" class="w-full"
                  hx-get="{{ route('catalog.show', ['collection' => \Illuminate\Support\Str::slug($collection)]) }}"
                    ...
                    <div class="flex flex-col gap-3">
                        ...
                        <div class="flex flex-row items-center gap-2">
                            ...
                            @if(!empty($selectedSport) || !empty($selectedTypes))
                                <a href="{{ route('catalog.show', ['collection' => \Illuminate\Support\Str::slug($collection)]) }}" class="px-3 sm:px-4 py-2 bg-white border border-slate-300 text-slate-700 text-xs font-bold uppercase tracking-wider rounded-lg hover:bg-slate-100 transition-colors shadow-sm whitespace-nowrap">
                                    Clear
                                </a>
                            @endif
                        </div>
                    ...
                    <div class="flex flex-col group">
                        ...
                        <div class="pt-3 flex flex-col text-center items-center w-full px-1">
                            ...
                            <div class="w-full flex items-center justify-center gap-3 mt-3 mb-1">
                                {{-- Facebook --}}
                                <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(route('catalog.show', ['collection' => \Illuminate\Support\Str::slug($collection), 'design' => $design->id])) }}" target="_blank" class="text-slate-400 hover:text-blue-600 transition-colors" title="Share on Facebook">
                                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.469h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.469h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                                </a>
                                {{-- Twitter --}}
                                <a href="https://twitter.com/intent/tweet?url={{ urlencode(route('catalog.show', ['collection' => \Illuminate\Support\Str::slug($collection), 'design' => $design->id])) }}&text={{ urlencode('Check out ' . $design->name . ' Design Concept') }}" target="_blank" class="text-slate-400 hover:text-sky-500 transition-colors" title="Share on Twitter">
                                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/></svg>
                                </a>
                                {{-- WhatsApp --}}
                                <a href="https://api.whatsapp.com/send?text={{ urlencode('Check out ' . $design->name . ' Design Concept: ' . route('catalog.show', ['collection' => \Illuminate\Support\Str::slug($collection), 'design' => $design->id])) }}" target="_blank" class="text-slate-400 hover:text-green-500 transition-colors" title="Share on WhatsApp">
                                ...
                                {{-- Copy Link --}}
                                <a href="#" @click.prevent="navigator.clipboard.writeText('{{ route('catalog.show', ['collection' => \Illuminate\Support\Str::slug($collection), 'design' => $design->id]) }}'); alert('Link copied for sharing!')" class="text-slate-400 hover:text-pink-600 transition-colors" title="Copy link for sharing">
